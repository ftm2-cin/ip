...
void carregarPartidas(Partida **partidas, int *total_partidas){
    ...
    while(!feof(arq)){
        ...
        Partida *temp = (*partidas);
        if((*partidas)==NULL){//Testa se o realloc funcionou corretamente
...
void geraTabela(Partida *partidas, int totalPartidas){
    ...
    for(int i=0;i<totalPartidas;i++){
        ...
        if(flagCasa==0){
            ...
            Time *temp = tabela;
            if(tabela==NULL){//Testa se o realloc funcionou corretamente
                printf("Erro de alocação de memória\n");
                free(temp);
                exit(1);
            }
    ...
    free(tabela);//Você pode dar o free dentro dessa função pois
}
