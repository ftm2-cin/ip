...
void carregarPartidas(Partida **partidas, int *total_partidas)
{
    ...
    while (!feof(arq))
    {
        ...
        while ((linha[cont] < '0') || (linha[cont] > '9'))
            cont++;                                           // Ao final do while, cont armazena a posição onde tem a quantidade de gols
...
void geraTabela(Partida *partidas, int totalPartidas)
{
    ...
    for (int i = 0; i < totalPartidas; i++)
    {
        ...
        {
            if (strcmp(partidas[i].time_de_casa, tabela[j].nome) == 0)
            { // Testa se o time da casa já tinha sido adicionado na tabela antes e atualiza os valores
            ...
            if (strcmp(partidas[i].visitante, tabela[j].nome) == 0)
            { // Testa se o time visitante já tinha sido adicionado na tabela antes e atualiza os valores
    ...
    for (int i = 0; i < qtd_times - 1; i++)
    {
        for (int j = 1; j < qtd_times; j++)
        {
            ...
            { // Caso os pontos sejam iguais
                if (tabela[j].gc > tabela[j - 1].gc)
                { // Comparamos por quantidade de gols feitos
                    temp = tabela[j];
                    tabela[j] = tabela[j - 1];
                    tabela[j - 1] = temp;
                }
...
int main()
{
    ...
    while (flag == 0)
    {
        ...
        case 1:
            if (leitura == 0)
            { // Na questão ele diz que a leitura só deve ser feita 1 vez
                leitura = 1;
